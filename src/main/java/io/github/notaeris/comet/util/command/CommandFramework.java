package io.github.notaeris.comet.util.command;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.playerOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', CometPlugin.get().getConfig().getString("command.executor")));
                    return true;
                }
                if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', CometPlugin.get().getConfig().getString("command.no_permission")));
                    return true;
                }
                try {
